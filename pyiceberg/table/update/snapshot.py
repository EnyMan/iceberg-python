...
class RewriteDataFiles(UpdateTableMetadata["RewriteDataFiles"]):
    ...
    def _commit(self) -> UpdatesAndRequirements:
        """Execute the rewrite operation and return updates.

        This method is called by the parent class's commit() method.
        Since we need to do the actual work and return a result, we override commit().
        """
        # This will be called but we handle the actual work in commit()
        return (), ()

    def commit(self) -> RewriteDataFilesResult:
        """Execute the rewrite operation and commit the changes.

        Returns:
            RewriteDataFilesResult with metrics about the operation.
        """
        # Get candidate files
