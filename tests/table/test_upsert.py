...
# ============================================================================
# Integration Test with Logging Verification
# ============================================================================


def test_coarse_match_filter_logs_warning_for_full_scan(caplog: pytest.LogCaptureFixture) -> None:
    """Verify logging when AlwaysTrue is used (indicates full scan will be performed)."""
    import logging

    from pyiceberg.table.upsert_util import LARGE_FILTER_THRESHOLD, create_coarse_match_filter

    # Create table with >10k sparse IDs (will trigger AlwaysTrue)
    num_keys = LARGE_FILTER_THRESHOLD
    # Very sparse IDs
    ids = list(range(0, num_keys * 1000, 1000))
    data = {"id": ids, "value": list(range(num_keys))}
    ...
    with caplog.at_level(logging.INFO, logger="pyiceberg.table.upsert_util"):
        result = create_coarse_match_filter(table, ["id"])

    # Verify AlwaysTrue is returned
    assert isinstance(result, AlwaysTrue)

    # Verify log message about skipping filter
    assert any("full scan will be performed" in record.message for record in caplog.records)


def test_coarse_match_filter_logs_range_filter_usage(caplog: pytest.LogCaptureFixture) -> None:
    """Verify logging when range filter is used for dense IDs."""
    import logging
    ...
    # Create table with >10k dense IDs (will use range filter)
    num_keys = LARGE_FILTER_THRESHOLD
    data = {"id": list(range(num_keys)), "value": list(range(num_keys))}
    schema = pa.schema([pa.field("id", pa.int64()), pa.field("value", pa.int64())])
    table = pa.Table.from_pydict(data, schema=schema)
    ...
    # Verify range filter is returned
    assert isinstance(result, And)
    assert isinstance(result.left, GreaterThanOrEqual)
    assert isinstance(result.right, LessThanOrEqual)

    # Verify log message about using range filter
    assert any("Using range filter" in record.message for record in caplog.records)


def test_coarse_match_filter_logs_large_dataset_detection(caplog: pytest.LogCaptureFixture) -> None:
    """Verify logging when large dataset is detected."""
    import logging

    from pyiceberg.table.upsert_util import LARGE_FILTER_THRESHOLD, create_coarse_match_filter

    # Create table with exactly LARGE_FILTER_THRESHOLD keys
    num_keys = LARGE_FILTER_THRESHOLD
    data = {"id": list(range(num_keys)), "value": list(range(num_keys))}
    schema = pa.schema([pa.field("id", pa.int64()), pa.field("value", pa.int64())])
    table = pa.Table.from_pydict(data, schema=schema)
    ...
    with caplog.at_level(logging.INFO, logger="pyiceberg.table.upsert_util"):
        create_coarse_match_filter(table, ["id"])

    # Verify log message about large dataset detection
    assert any("Large dataset detected" in record.message for record in caplog.records)
    assert any(f"{LARGE_FILTER_THRESHOLD}" in record.message for record in caplog.records)
